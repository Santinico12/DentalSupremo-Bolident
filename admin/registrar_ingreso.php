<?php
/**
 * Registrar Ingreso General (No vinculado a presupuestos)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $registradoPor = htmlspecialchars($_SESSION['user']['username'] ?? 'admin');
    $fecha = $_POST['fecha'] . ' ' . date('H:i:s');

    if (empty($nombre) || $monto <= 0 || empty($metodoPago)) {
        ...
        $ingresoId = $cajaModel->crearOtroIngreso($dataIngreso);
        ...
        if ($ingresoId) {
            ...
            if (isset($_FILES['comprobante']) && $_FILES['comprobante']['error'] === UPLOAD_ERR_OK) {
                ...
                if ($compFile['size'] > $maxSize) {
                    $_SESSION['message'] = 'Ingreso registrado pero el comprobante supera los 10MB';
                    $_SESSION['message_type'] = 'warning';
                } elseif (!$esValido) {
                    $_SESSION['message'] = 'Ingreso registrado pero el comprobante debe ser PDF o imagen válida';
                    $_SESSION['message_type'] = 'warning';
                } else {
                    // Guardar comprobante
                    ...
                    $nombreArchivo = 'ingreso_' . $ingresoId . '_' . date('Ymd_His') . '.' . $ext;
                    $ruta = $uploadDir . $nombreArchivo;

                    if (move_uploaded_file($compFile['tmp_name'], $ruta)) {
                        $cajaModel->actualizarComprobanteOtroIngreso($ingresoId, $ruta, $ext);
                        $_SESSION['message'] = 'Ingreso registrado con éxito';
                        $_SESSION['message_type'] = 'success';
                    } else {
                        $_SESSION['message'] = 'Ingreso registrado pero falló la subida del comprobante';
                        $_SESSION['message_type'] = 'warning';
                    }
                }
            } else {
            ...
            $mensaje = 'Error al registrar el ingreso en la base de datos';
            $tipoMensaje = 'danger';
        }
    }
}
...
<style>
    ...
    .metodo-option label { display: block; padding: 15px; border: 2px solid #e9ecef; border-radius: 10px; text-align: center; cursor: pointer; transition: all 0.2s; font-weight: 600; }
    .metodo-option label i { font-size: 1.5rem; display: block; margin-bottom: 8px; }
    .metodo-option input:checked + label { border-color: #28a745; background: #e2f0d9; color: #385723; }

    .btn-submit { background: #28a745; color: white; border: none; padding: 15px 40px; border-radius: 10px; font-size: 1.1rem; font-weight: 700; cursor: pointer; width: 100%; display: flex; align-items: center; justify-content: center; gap: 10px; transition: background 0.2s; }
...
<div class="page-container">
    ...
    <div class="page-header">
        <h1><i class="fas fa-plus-circle"></i> Registrar Ingreso General</h1>
        <a href="caja.php" class="btn-back">
            <i class="fas fa-arrow-left"></i> Volver a Caja
        </a>
    </div>

    <form method="POST" enctype="multipart/form-data" id="formIngreso">
        <!-- Datos Principales -->
        <div class="form-card" style="border-left: 4px solid #28a745;">
            <h3><i class="fas fa-info-circle text-success"></i> Detalles del Ingreso</h3>
            
            <div class="form-group">
                <label>Concepto / Nombre del Ingreso <span class="required">*</span></label>
                <input type="text" name="nombre" class="form-control" placeholder="Ej: Venta de kits dentales, Alquiler de consultorio, Curso..." required>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="descripcion" class="form-control" rows="3" placeholder="Información adicional del ingreso..."></textarea>
            </div>
        </div>

        <!-- Método de Pago / Comprobante -->
        <div class="form-card">
            <h3><i class="fas fa-wallet text-success"></i> Método de Pago y Comprobante</h3>

            <div class="form-group">
                <label>Método de Pago <span class="required">*</span></label>
                <div class="metodo-group">
                    <div class="metodo-option">
                        <input type="radio" name="metodo_pago" id="metodoEfectivo" value="efectivo" checked>
                        <label for="metodoEfectivo">
                            <i class="fas fa-money-bill-wave"></i> Efectivo
                        </label>
                    </div>
                    <div class="metodo-option">
                        <input type="radio" name="metodo_pago" id="metodoQR" value="qr">
                        <label for="metodoQR">
                            <i class="fas fa-qrcode"></i> Pago QR
                        </label>
                    </div>
                    <div class="metodo-option">
                        <input type="radio" name="metodo_pago" id="metodoTrans" value="transferencia">
                        <label for="metodoTrans">
            ...
            <div class="form-group" style="margin-top: 25px;">
                <label>Comprobante de Ingreso (Opcional)</label>
                <div class="dropzone" id="dropzone" onclick="document.getElementById('fileInput').click()">
                    <input type="file" name="comprobante" id="fileInput" style="display: none;" accept="image/*,application/pdf">
                    <div id="dropContent">
                        <i class="fas fa-cloud-upload-alt text-success" style="font-size: 2.5rem; margin-bottom: 10px;"></i>
                    ...
                    <div id="previewContainer" style="display: none; text-align: center;">
                        ...
                        <div id="pdfPreview" style="display: none; padding: 15px; background: #e2f0d9; border-radius: 8px; margin-bottom: 10px;">
                            <i class="fas fa-file-pdf text-success" style="font-size: 2.5rem; margin-bottom: 8px;"></i>
        ...
        <button type="submit" class="btn-submit">
            <i class="fas fa-check-circle"></i> Guardar Ingreso
        </button>
    </form>
</div>
